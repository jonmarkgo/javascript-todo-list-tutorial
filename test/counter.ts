...
function view(model: number, signal: (action: Action) => () => void): HTMLElement {
...
function mount(model: number, update: (action: Action, model: number) => number, view: (model: number, signal: (action: Action) => () => void) => HTMLElement, root_element_id: string): void {
