...
import * as fs from 'fs';      // read html files (see below)
import * as path from 'path';  // so we can open files cross-platform
import * as elmish from '../lib/elmish';
import { add_attributes, empty, mount } from '../lib/elmish';
import { view, update } from '../test/counter';
import { view as counterView, update as counterUpdate } from './counter';
import jsdomGlobal from 'jsdom-global';

const htmlContent = fs.readFileSync(path.resolve(__dirname, '../index.html'));
jsdomGlobal(htmlContent);   // https://github.com/rstacruz/jsdom-global

const testId = 'app'; // Define the id variable at the beginning of the file

test('elmish.empty("root") removes DOM elements from container', function (t) {
  // setup the test div:
  const text = 'Hello World!'
  const divid = "mydiv";
  const root = document.getElementById(testId);
  if (!root) {
  ...
  t.equal(root.childElementCount, 1, "Root element " + testId + " has 1 child el");
...
test('elmish.mount app expect state to be Zero', function (t) {
  ...
  mount(initial_model, counterUpdate as any, counterView as any, id);
...
test('elmish create "main" view using HTML DOM functions', function (t) {
  ...
  if (rootElement) {
    ...
    const initial_model = { todos: [], hash: '#/' };
    mount(initial_model, mockUpdate, mockView, id);
...
test('elmish create <footer> view using HTML DOM functions', function (t) {
  ...
  if (rootElement) {
    ...
    add_attributes(["class=footer", "style=display: block;"], footerElement);

    const spanElement = document.createElement('span');
    add_attributes(["class=todo-count", "id=count"], spanElement);

    const strongElement = document.createElement('strong');
    strongElement.textContent = "1";
    spanElement.appendChild(strongElement);
    spanElement.appendChild(createTextElement(" item left"));

    const ulElement = ul(["class=filters"], [
      li([], [
    ...
    add_attributes(["class=clear-completed", "style=display:block;"], buttonElement);
    buttonElement.appendChild(createTextElement("Clear completed"));
...
// Use the previously declared testId instead of redeclaring id
localStorage.removeItem('todos-elmish_' + testId);
...
test('elmish.mountApp sets model in localStorage', function (t) {
  ...
  const initial_model = { todos: [], hash: '#/' };
  mount(initial_model, update, view, testId);
  // the "model" stored in localStorage should be 7 now:
  const storedValue = localStorage.getItem('todos-elmish_' + testId);
  ...
  mount(different_model, update, view, testId); // model should be ignored this time!
  ...
  mount(different_model, update, view, id); // different_model ignored! read model from localStorage
...
test('elmish.mount app expect state to be Zero', (t: test.Test) => {
  ...
  const root = document.getElementById(id) as HTMLElement;
  elmish.mount({ todos: [], hash: '#/' }, update, view, id);
  const actual = document.getElementById(id)?.textContent;
  const actual_stripped = parseInt(actual?.replace('+', '')
    .replace('-Reset', '') ?? '', 10);
  const expected = 0;
...
test('test elmish.add_attributes attrlist null (no effect)', function (t) {
  ...
  // "Clone" the div DOM node before invoking elmish.attributes to compare
  const clone = div.cloneNode(true) as HTMLDivElement;
  div = elmish.add_attributes([] as (string | ((this: GlobalEventHandlers, ev: MouseEvent) => any))[], div); // should not "explode"
  t.deepEqual(div, clone, "<div> has not been altered");
  t.end();
});
...
test('elmish.mount sets model in localStorage', function (t) {
  ...
  if (root) {
    elmish.mount({ todos: [], hash: '#/' }, update, view, id);
    // the "model" stored in localStorage should be { todos: [], hash: '#/' } now:
    const storedValue = localStorage.getItem('todos-elmish_' + id);
    t.deepEqual(JSON.parse(storedValue || '{}'), { todos: [], hash: '#/' },
      "todos-elmish_store is { todos: [], hash: '#/' } (as expected). initial state saved to localStorage.");
    // test that mount still works as expected (check initial state of counter):
    ...
    // attempting to "re-mount" with a different model value should not work
    // because mount should retrieve the value from localStorage
    elmish.mount({ todos: [{ id: 1, title: 'Test', completed: false }], hash: '#/active' }, update, view, id); // model should be ignored this time!

    const storedValue2 = localStorage.getItem('todos-elmish_' + id);
    t.deepEqual(JSON.parse(storedValue2 || '{}'), { todos: [], hash: '#/' },
      "todos-elmish_store is { todos: [], hash: '#/' } (as expected). initial state saved to localStorage.");
    // increment the counter
    const btn = root.getElementsByClassName("inc")[0] as HTMLElement; // click increment button
    if (btn) {
    ...
    // the "model" stored in localStorage should also be updated now:
    const storedValue3 = localStorage.getItem('todos-elmish_' + id);
    t.deepEqual(JSON.parse(storedValue3 || '{}'), { todos: [], hash: '#/', count: 1 },
      "todos-elmish_store is updated (as expected).");
    elmish.empty(root); // reset the DOM to simulate refreshing a browser window
    elmish.mount({ todos: [], hash: '#/' }, update, view, id); // should read model from localStorage
    // clearing DOM does NOT clear the localStorage (this is desired behaviour!)
    const storedValue4 = localStorage.getItem('todos-elmish_' + id);
    t.deepEqual(JSON.parse(storedValue4 || '{}'), { todos: [], hash: '#/', count: 1 },
      "todos-elmish_store still has the updated state from increment (above) saved in localStorage");
  }
  localStorage.removeItem('todos-elmish_' + id);
  t.end();
});
