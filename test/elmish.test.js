...
var fs_1 = __importDefault(require("fs")); // read html files (see below)
var path_1 = __importDefault(require("path")); // so we can open files cross-platform
var elmish = __importStar(require("../lib/elmish"));
...
(0, tape_1.default)('elmish.empty("root") removes DOM elements from container', function (t) {
    var _a;
    // setup the test div:
...
(0, tape_1.default)('elmish.mount app expect state to be Zero', function (t) {
    var _a, _b, _c;
...
(0, tape_1.default)('elmish.add_attributes adds "autofocus" attribute', function (t) {
    var _a;
...
(0, tape_1.default)('elmish.add_attributes applies HTML class attribute to el', function (t) {
    var root = document.getElementById(id);
    var div = document.createElement('div');
    div.id = 'divid';
...
(0, tape_1.default)('elmish.add_attributes applies id HTML attribute to a node', function (t) {
    var _a;
    var root = document.getElementById(id);
...
(0, tape_1.default)('elmish.add_attributes applies multiple attribute to node', function (t) {
    var _a;
    var root = document.getElementById(id);
    elmish.empty(root);
...
(0, tape_1.default)('elmish.add_attributes set placeholder on <input> element', function (t) {
    var _a;
    var root = document.getElementById(id);
...
(0, tape_1.default)('elmish.add_attributes set data-id on <li> element', function (t) {
    var _a;
    var root = document.getElementById(id);
...
(0, tape_1.default)('elmish.add_attributes set "for" attribute <label> element', function (t) {
    var _a;
    var root = document.getElementById(id);
    var li = document.createElement('li');
    li.id = 'toggle';
...
(0, tape_1.default)('elmish.add_attributes type="checkbox" on <input> element', function (t) {
    var _a;
    var root = document.getElementById(id);
    var input = document.createElement('input');
...
(0, tape_1.default)('elmish.add_attributes apply style="display: block;"', function (t) {
    var root = document.getElementById(id);
    elmish.empty(root);
...
(0, tape_1.default)('elmish.add_attributes checked=true on "done" item', function (t) {
    var root = document.getElementById(id);
    elmish.empty(root);
    var input = document.createElement('input');
...
(0, tape_1.default)('elmish.add_attributes <a href="#/active">Active</a>', function (t) {
    var root = document.getElementById(id);
    elmish.empty(root);
...
/** DEFAULT BRANCH **/
(0, tape_1.default)('test default branch of elmish.add_attributes (no effect)', function (t) {
    var root = document.getElementById(id);
    var div = document.createElement('div');
    div.id = 'divid';
...
/** null attrlist **/
(0, tape_1.default)('test elmish.add_attributes attrlist null (no effect)', function (t) {
    var root = document.getElementById(id);
    var div = document.createElement('div');
    div.id = 'divid';
    // "Clone" the div DOM node before invoking elmish.attributes to compare
...
(0, tape_1.default)('elmish.append_childnodes append child DOM nodes to parent', function (t) {
    var root = document.getElementById(id);
...
(0, tape_1.default)('elmish.section creates a <section> HTML element', function (t) {
    var p = document.createElement('p');
    p.id = 'para';
    var text = 'Hello World!';
    var txt = document.createTextNode(text);
    ...
    // document.activeElement via: https://stackoverflow.com/a/17614883/1148249
    t.equal(document.getElementById('para').textContent, text, '<section> <p>' + text + '</p></section> works as expected!');
    elmish.empty(document.getElementById(id));
    t.end();
});
(0, tape_1.default)('elmish create <header> view using HTML element functions', function (t) {
    ...
    var place = document.getElementById('new').getAttribute('placeholder');
...
(0, tape_1.default)('elmish create "main" view using HTML DOM functions', function (t) {
    ...
    var completed = document.querySelectorAll('.completed')[0].textContent;
    t.equal(completed, 'Learn The Elm Architecture ("TEA")', 'Completed: Learn "TEA"');
    var todo = document.querySelectorAll('.view')[1].textContent;
    t.equal(todo, 'Build TEA Todo List App', 'Todo: Build TEA Todo List App');
    elmish.empty(document.getElementById(id));
    t.end();
});
(0, tape_1.default)('elmish create <footer> view using HTML DOM functions', function (t) {
    ...
    // count of items left:
    var left = document.getElementById('count').textContent;
...
(0, tape_1.default)('elmish.route updates the url hash and sets history', function (t) {
    ...
    var state = { hash: '' };
    var new_hash = '#/active';
    var new_state = elmish.route(state, 'Active', new_hash);
...
(0, tape_1.default)('elmish.mount sets model in localStorage', function (t) {
    var _a = require('./counter.js'), view = _a.view, update = _a.update;
    var root = document.getElementById(id);
    elmish.mount(7, update, view, id);
    // the "model" stored in localStorage should be 7 now:
    t.equal(JSON.parse(localStorage.getItem('todos-elmish_' + id)), 7, "todos-elmish_store is 7 (as expected). initial state saved to localStorage.");
    ...
    var state = parseInt(root.getElementsByClassName('count')[0]
        .textContent, 10);
    t.equal(state, 8, "State is 8 after increment.");
    // the "model" stored in localStorage should also be 8 now:
    t.equal(JSON.parse(localStorage.getItem('todos-elmish_' + id)), 8, "todos-elmish_store is 8 (as expected).");
    ...
    t.equal(JSON.parse(localStorage.getItem('todos-elmish_' + id)), 8, "todos-elmish_store still 8 from increment (above) saved in localStorage");
    localStorage.removeItem('todos-elmish_' + id);
    t.end();
});
(0, tape_1.default)('elmish.add_attributes onclick=signal(action) events!', function (t) {
    var root = document.getElementById(id);
    elmish.empty(root);
...
(0, tape_1.default)('subscriptions test using counter-reset-keyaboard ⌨️', function (t) {
    ...
    t.equal(JSON.parse(localStorage.getItem('todos-elmish_' + id)), 0, "todos-elmish_store is 0 (as expected). initial state saved to localStorage.");
    ...
    t.equal(JSON.parse(localStorage.getItem('todos-elmish_' + id)), 1, "todos-elmish_store 1 (as expected). incremented state saved to localStorage.");
    ...
    t.equal(JSON.parse(localStorage.getItem('todos-elmish_' + id)), 0, "todos-elmish_store 0. keyboard down key decrement state saved to localStorage.");
