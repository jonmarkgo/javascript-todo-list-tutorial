...
import { Action, AppModel, Todo } from '../todo-app.js';
import { JSDOM } from 'jsdom';
import jsdomGlobal from 'jsdom-global';

// JSDOM setup
// This creates a mock DOM environment for our tests to run in
const cleanup = jsdomGlobal('<!DOCTYPE html><html><body></body></html>', {
  url: 'http://localhost',
  runScripts: 'dangerously'
}) as () => void;

// Ensure global.window and global.document are properly typed
declare global {
...
describe('update function', () => {
  it('should return model unmodified for unknown action', () => {
    const model: AppModel = JSON.parse(JSON.stringify(app.model));
    const unmodified_model = app.update('UNKNOWN_ACTION' as Action, model, '');
    expect(model).to.deep.equal(unmodified_model);
  });

  it('should add a new todo item to model.todos Array', () => {
    const model: AppModel = JSON.parse(JSON.stringify(app.model)); // initial state
...
describe('render_item', () => {
  it('should render HTML for a single Todo Item', () => {
    ...
    if (container) {
      ...
      expect(doneElement?.textContent).to.equal('Learn Elm Architecture');
  ...
  it('should render "main" view using (elmish) HTML DOM functions', () => {
    ...
    if (container) {
      ...
      viewElements.forEach((item, index) => {
        expect(item.textContent).to.equal(model.todos[index].title,
      ...
      [true, false, false].forEach((state, index) => {
        ...
        expect(input.checked).to.equal(state,
...
describe('view', () => {
  it('renders the whole todo app using "partials"', () => {
    ...
    if (container) {
      ...
      expect(h1Element?.textContent).to.equal("todos");
      ...
      const countElement = document.getElementById('count');
      expect(countElement).to.exist;
      expect(countElement?.innerHTML).to.equal("<strong>0</strong> items left");
    }
...
describe('4. Item', () => {
  it('should allow me to mark items as complete', () => {
    ...
    if (container) {
      ...
      expect(item?.textContent).to.equal(model.todos[0].title, 'Item contained in model.');
...
describe('4.1 DELETE item', () => {
  it('should delete an item by clicking <button class="destroy">', () => {
    ...
    if (container) {
      ...
      const item = document.getElementById('0');
      expect(item).to.exist;
      expect(item?.textContent).to.equal(model.todos[0].title, 'Item contained in DOM.');
      ...
      expect(document.querySelectorAll('button.destroy').length).to.equal(0,
...
describe('5.1 Editing', () => {
  it('should render an item in "editing mode"', () => {
    ...
    if (container) {
      ...
      expect(document.querySelectorAll('.editing').length).to.equal(1,
        "<li class='editing'> element is visible");
      expect(document.querySelectorAll('.edit').length).to.equal(1,
      ...
      expect(editInput.value).to.equal(model.todos[2].title,
        "<input class='edit'> has correct value");
    }
  });
});
...
describe('5.2 Double-click an item <label> to edit it', () => {
  it('should enter editing mode on double-click', () => {
    ...
    const editInput = document.querySelector('.edit') as HTMLInputElement;
    expect(editInput).to.exist;
    expect(editInput.value).to.equal(model.todos[1].title,
      "<input class='edit'> has correct value");
  });
});
...
describe('5.2.2 Slow clicks do not count as double-click', () => {
  it('should not enter editing mode on slow clicks', (done) => {
    ...
    setTimeout(() => {
      ...
      expect(document.querySelectorAll('.editing').length).to.equal(0,
...
describe('5.3 [ENTER] Key in edit mode triggers SAVE action', () => {
  it('should save the edited todo item on ENTER key', () => {
    ...
    expect(label.textContent).to.equal(updated_title.trim(),
      `item title updated to: ${updated_title.trim()}`);
  });
});
...
describe('5.5 CANCEL should cancel edits on escape', () => {
  it('should cancel edits when ESC key is pressed', () => {
    ...
    expect(originalLabel.textContent).to.equal(model.todos[1].title,
    ...
    expect(newLabel.textContent).to.equal(model.todos[1].title,
      `todo id 1 has title: ${model.todos[1].title}`);
    localStorage.removeItem('todos-elmish_' + id);
  });
});
...
describe('7. Clear Completed', () => {
  it('should display the number of completed items', () => {
    ...
    expect(document.querySelectorAll('.view').length).to.equal(3,
    ...
    expect(completed_count).to.equal(done_count,
    ...
    expect(document.querySelectorAll('.view').length).to.equal(1,
    ...
    expect(document.querySelectorAll('.clear-completed').length).to.equal(0,
...
describe('8. Persistence', () => {
  it('should persist its data', () => {
    ...
    expect(localStorage.getItem('todos-elmish_' + id)).to.equal(
...
describe('9. Routing', () => {
  it('should allow me to display active/completed/all items', () => {
    ...
    elmish.mount(model, app.update, app.view, id, app.subscriptions);
    expect(document.querySelectorAll('.view').length).to.equal(2,
      "two completed items");
