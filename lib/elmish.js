 ...
 */
export function mountApp(model, update, view, root_element_id, subscriptions) {
    var ROOT = document.getElementById(root_element_id); // root DOM element
    if (!ROOT)
        throw new Error("Element with id ".concat(root_element_id, " not found"));
...
export var empty = emptyNode;
// Alias for mountApp to maintain compatibility
export var mount = mountApp;
export function button(attrs, children) {
    var processedChildren = children.map(function (child) { return typeof child === 'string' ? document.createTextNode(child) : child; });
    return create_element('button', attrs, processedChildren);
}
export function div(attrs, children) {
    var processedChildren = children.map(function (child) { return typeof child === 'string' ? document.createTextNode(child) : child; });
    return create_element('div', attrs, processedChildren);
}
...
function create_element(type, attrlist, childnodes) {
    var element = add_attributes(attrlist, document.createElement(type));
...
export function section(attrlist, childnodes) {
    var processedChildren = childnodes.map(function (child) { return typeof child === 'string' ? document.createTextNode(child) : child; });
