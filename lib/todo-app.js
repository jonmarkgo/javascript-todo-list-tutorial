import { a, button, div, footer, input, h1, header, label, li, section, span, strong, text, ul } from './elmish.js';
export var initial_model = {
    todos: [],
    hash: "#/",
    all_done: false
};
...
export function update(action, model, data) {
    ...
    switch (action) {
        ...
        case 'SAVE':
            ...
            var value_1 = edit.value;
            var editId_1 = parseInt(edit.id, 10);
            // End Editing
            new_model.clicked = undefined;
            new_model.editing = undefined;
            if (!value_1 || value_1.length === 0) { // delete item if title is blank:
                return update('DELETE', new_model, editId_1);
            }
            // update the value of the item.title that has been edited:
            new_model.todos = new_model.todos.map(function (item) {
                if (item.id === editId_1 && value_1 && value_1.length > 0) {
                    item.title = value_1.trim();
                }
