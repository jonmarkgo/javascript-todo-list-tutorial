...
export function render_item(todo: Todo, model: TodoModel, signal: Signal): HTMLElement {
  ...
  toggle.onclick = () => signal({ type: 'TOGGLE', id: todo.id } as Action);
  ...
  button.onclick = () => signal({ type: 'DELETE', id: todo.id } as Action);
  ...
  edit.onkeyup = (e) => {
    if (e.key === 'Enter' && edit.value.trim()) {
      signal({ type: 'UPDATE', id: todo.id, title: edit.value } as Action);
      li.className = '';
    }
    if (e.key === 'Escape') {
      li.className = '';
...
export function render_main(model: TodoModel, signal: Signal): HTMLElement {
  ...
  toggle_all.onclick = () => signal({ type: 'TOGGLE_ALL' } as Action);
...
export function render_footer(model: TodoModel): HTMLElement {
...
export function view(model: TodoModel): HTMLElement {
  const div = document.createElement('div');
...
export let model: TodoModel = {
  todos: [],
  visibility: 'all',
  hash: ''
};

export function update(msg: Action, model: TodoModel): TodoModel {
  switch (msg.type) {
    case 'ADD':
      return {
        ...model,
...
export function subscriptions(signal: Signal): void {
  window.addEventListener('hashchange', () => {
    signal({ type: 'SET_VISIBILITY', filter: window.location.hash.slice(2) || 'all' } as Action);
  });
}
