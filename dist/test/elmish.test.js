...
test('elmish.empty("root") removes DOM elements from container', function (t) {
    ...
    const actual = (_a = document.getElementById(divid)) === null || _a === void 0 ? void 0 : _a.textContent;
...
test('elmish.mount app expect state to be Zero', function (t) {
    ...
    const actual = (_a = document.getElementById(id)) === null || _a === void 0 ? void 0 : _a.textContent;
    const actual_stripped = parseInt((_b = actual === null || actual === void 0 ? void 0 : actual.replace('+', '').replace('-Reset', '')) !== null && _b !== void 0 ? _b : '', 10);
    const expected = 7;
    ...
    const state = parseInt((_c = countElement.textContent) !== null && _c !== void 0 ? _c : '', 10);
...
test('elmish.add_attributes set placeholder on <input> element', function (t) {
    ...
    const placeholder = (_a = document.getElementById('new-todo')) === null || _a === void 0 ? void 0 : _a.getAttribute("placeholder");
...
test('elmish.add_attributes set data-id on <li> element', function (t) {
    ...
    root.appendChild(li);
    const data_id = (_a = document.getElementById('task1')) === null || _a === void 0 ? void 0 : _a.getAttribute("data-id");
...
test('elmish.add_attributes set "for" attribute <label> element', function (t) {
    ...
    label.id = 'toggle';
    label = add_attributes(["for=toggle-all"], label);
    root.appendChild(label);
    const label_for = (_a = document.getElementById('toggle')) === null || _a === void 0 ? void 0 : _a.getAttribute("for");
...
test('elmish.add_attributes type="checkbox" on <input> element', function (t) {
    ...
    let input = document.createElement('input');
    input = add_attributes(["type=checkbox", "id=toggle-all"], input);
    root.appendChild(input);
    const type_atrr = (_a = document.getElementById('toggle-all')) === null || _a === void 0 ? void 0 : _a.getAttribute("type");
...
test('elmish.add_attributes apply style="display: block;"', function (t) {
    ...
    root.appendChild(add_attributes(["id=main", "style=display: block;"], sec));
...
test('elmish.add_attributes checked=true on "done" item', function (t) {
    ...
    elmish.empty(root);
    let input = document.createElement('input');
    input = add_attributes(["type=checkbox", "id=item1", "checked=true"], input);
    root.appendChild(input);
    ...
    root.appendChild(add_attributes(["type=checkbox", "id=item2"], document.createElement('input')));
...
test('elmish.add_attributes <a href="#/active">Active</a>', function (t) {
    ...
    elmish.empty(root);
    root.appendChild(add_attributes(["href=#/active", "class=selected", "id=active"], document.createElement('a')));
...
test('elmish create <header> view using HTML element functions', function (t) {
    ...
    if (rootElement) {
        ...
        t.equal(h1Element === null || h1Element === void 0 ? void 0 : h1Element.textContent, 'todos', '<h1>todos</h1>');
        elmish.empty(rootElement);
    }
    else {
        t.fail('Root element not found');
...
test('elmish create "main" view using HTML DOM functions', function (t) {
    ...
    if (rootElement) {
        ...
        const mockView = (model, signal) => document.createElement('div');
        const mockUpdate = (action, model) => model;
        const initial_model = { todos: [], hash: '#/' };
        elmish.mount(initial_model, mockUpdate, mockView, id);
        const done = (_a = document.querySelectorAll('.completed')[0]) === null || _a === void 0 ? void 0 : _a.textContent;
        t.equal(done, 'Learn The Elm Architecture ("TEA")', 'Done: Learn "TEA"');
        const todo = (_b = document.querySelectorAll('.view')[1]) === null || _b === void 0 ? void 0 : _b.textContent;
    ...
    if (rootElement) {
        elmish.append_childnodes([
            footer(["class=footer", "style=display: block;"], [
                ...
                elmish.button(["class=clear-completed", "style=display:block;"], [createTextElement("Clear completed")])
            ])
        ], rootElement);
        // count of items left:
        const left = (_a = document.getElementById('count')) === null || _a === void 0 ? void 0 : _a.textContent;
        t.equal("1 item left", left, 'there is 1 (ONE) todo item left');
        const clear = (_b = document.querySelectorAll('button')[0]) === null || _b === void 0 ? void 0 : _b.textContent;
        t.equal(clear, "Clear completed", '<button> text is "Clear completed"');
        const selected = (_c = document.querySelectorAll('.selected')[0]) === null || _c === void 0 ? void 0 : _c.textContent;
...
test('elmish.mount sets model in localStorage', function (t) {
    ...
    t.deepEqual(JSON.parse(storedValue || '{}'), initial_model, "todos-elmish_store is initial_model (as expected). initial state saved to localStorage.");
    ...
    t.deepEqual(JSON.parse(storedValue2 || '{}'), initial_model, "todos-elmish_store is still initial_model (as expected). initial state saved to localStorage.");
    // increment the counter
    const btn = root === null || root === void 0 ? void 0 : root.getElementsByClassName("inc")[0]; // click increment button
    if (btn) {
        btn.click(); // Click the Increment button!
    }
    const countElement = root === null || root === void 0 ? void 0 : root.getElementsByClassName('count')[0];
    ...
    t.deepEqual(JSON.parse(storedValue4 || '{}'), updatedModel, "todos-elmish_store still has the updated model from increment (above) saved in localStorage");
    localStorage.removeItem('todos-elmish_' + id);
    t.end();
});
test('elmish.add_attributes onclick=signal(action) events!', function (t) {
    ...
    root.appendChild(add_attributes(["id=btn", signal('inc')], document.createElement('button')));
    // "click" the button!
...
test('subscriptions test using counter-reset-keyaboard ⌨️', function (t) {
    ...
    const initial_model = { todos: [], hash: '#/' };
    elmish.mount(initial_model, update, view, id, subscriptions);
    // counter starts off at 0 (zero):
    const countElement = document.getElementById('count');
    t.equal(countElement === null || countElement === void 0 ? void 0 : countElement.textContent, '0', "counter starts at 0");
    // simulate keypress event:
    const event = new KeyboardEvent('keyup', { 'key': 'ArrowUp' });
    document.dispatchEvent(event);
    // counter should now be 1:
    t.equal(countElement === null || countElement === void 0 ? void 0 : countElement.textContent, '1', "counter incremented to 1 after ArrowUp keypress");
    // cleanup
    elmish.empty(root);
    t.end();
});
test('counter functionality test', function (t) {
    ...
    t.equal(JSON.parse(storedValueUp || '0'), 1, "todos-elmish_store 1 (as expected). incremented state saved to localStorage.");
    ...
    t.equal(JSON.parse(storedValueDown || '0'), 0, "todos-elmish_store 0. keyboard down key decrement state saved to localStorage.");
